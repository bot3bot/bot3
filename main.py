...
PROMOTION_PANEL_CHANNEL = 1497203612432990259  # روم لوحة طلب الترقية الموحد

# الترقية ورومات الاعتماد
...
# رتب الإدارة المعتمدة لفتح لوحات الإشراف من الشات
ADMIN_ROLES = {
    1478970736717598840,
    1495873706923393205,
    1490386915629989948,
...
# رتب الإدارة المعتمدة للترقيات بالتسلسل (من الأولى إلى التاسعة)
PROMOTION_ROLES = [
...
async def send_continuous_promotion_panel(guild: discord.Guild):
    ...
    await channel.send(embed=embed, view=PromotionPanel())
...
class LeaveModal(discord.ui.Modal, title="طلب إجازة"):
    reason = discord.ui.TextInput(label="سبب الإجازة", placeholder="اكتب السبب باختصار", required=True, max_length=200)
...
class PromotionDecisionView(discord.ui.View):
    ...
    async def accept_promo(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            await member.send(f"🎉 تهانينا! تم قبول طلب ترقيتك بنجاح إلى رتبة **{next_role.name if next_role else 'الإدارية الجديدة'}**.")
        except discord.HTTPException:
            pass

        await interaction.response.send_message("✅ تم قبول الترقية بنجاح.", ephemeral=True)
...
class PromotionPanel(discord.ui.View):
    ...
    async def request_promotion_isolated(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if req_channel:
            ...
            embed.add_field(name="🔺 الرتبة الحالية", value=current_role.mention if current_role else "غير محدد", inline=True)
            embed.add_field(name="🎯 الرتبة المطلوبة", value=next_role.mention if next_role else "غير محدد", inline=True)
            embed.set_thumbnail(url=member.display_avatar.url)
...
class AddPointsModal(discord.ui.Modal, title="إضافة نقاط"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.response.send_message("❌ البيانات المدخلة غير صحيحة.", ephemeral=True)
            return
        ...
        if not is_points_member(member):
            await interaction.response.send_message("❌ هذا العضو مستثنى أو لا يملك رتب التفاعل المعتمدة.", ephemeral=True)
            return

        total, req = add_points_to_user(member.id, amount, interaction.guild)
        await interaction.response.send_message(f"✅ تم إضافة {amount} نقطة إلى {member.mention}.", ephemeral=True)
...
async def interaction_panel_cmd(ctx: commands.Context):
    if ctx.channel.id != INTERACTION_PANEL_CHANNEL or not is_admin(ctx.author):
        return
    embed = discord.Embed(title="لوحة إدارة التفاعل والأعضاء العامة", description="إدارة نقاط التفاعل والتحكم بالدبل.", color=discord.Color.dark_teal())
...
async def promotion_panel_isolated_cmd(ctx: commands.Context):
    if ctx.channel.id != PROMOTION_PANEL_CHANNEL or not is_admin(ctx.author):
        return
    await send_continuous_promotion_panel(ctx.guild)
...
async def handle_message_points(message: discord.Message):
    ...
    has_image = any(attachment.content_type and attachment.content_type.startswith("image/") for attachment in message.attachments)
    ...
    if keyword_amount:
        ...
        embed_reply = discord.Embed(
            ...
            description=f"تم رصد تسجيل مادة تفاعلية صالحة واحتساب النقاط تلقائياً.",
...
async def check_monthly_activity():
    ...
    for guild in bot.guilds:
        ...
        for member in guild.members:
            if member.bot or has_any_role(member, {EXEMPTED_ACTIVITY_ROLE}) or not has_any_role(member, POINT_ROLES):
                continue
            ...
            if user_points < 500:
                if punish_role:
                    try:
                        await member.add_roles(punish_role, reason="إنذار تلقائي شهري")
                    except discord.HTTPException:
                        pass

                activity_warnings[uid] = {"guild_id": guild.id, "expires_at": expires_at.timestamp()}
        ...
        if current_timestamp >= data["expires_at"]:
            ...
            if guild:
                ...
                if member and punish_role:
                    try:
                        await member.remove_roles(punish_role, reason="انتهاء مدة العقوبة")
                        warning_channel = guild.get_channel(ACTIVITY_WARNING_CHANNEL)
                        if warning_channel:
                            embed = discord.Embed(title="✅ انتهاء عقوبة الإنذار", description=f"تم سحب رتبة العقوبة تلقائياً عن العضو {member.mention}.", color=discord.Color.green())
                            await asyncio.sleep(2.5)
...
DURATION_RE = re.compile(r"(?P<number>\d+)\s*(?P<unit>دقيقة|دقائق|د|ساعة|ساعه|ساعات|س|يوم|ايام|أيام|d|h|m)", re.IGNORECASE)
...
async def create_warning(guild: discord.Guild, target: discord.Member, moderator: discord.Member, reason: str, duration: datetime.timedelta, source_message_id: int | None = None):
    ...
    warnings.setdefault(str(target.id), []).append({
        "id": warning_id, "user_id": target.id, "moderator_id": moderator.id, "reason": reason,
        "created_at": created_at.timestamp(), "expires_at": expires_at.timestamp(), "source_message_id": source_message_id
    })
    ...
    try:
        await target.timeout(created_at + duration, reason=reason)
    except discord.Forbidden:
        pass
...
class WarningCreateModal(discord.ui.Modal, title="تسجيل إنذار"):
    ...
    duration = discord.ui.TextInput(label="مدة الإنذار", placeholder="مثال: 3 أيام", required=True, max_length=30)

    async def on_submit(self, interaction: discord.Interaction):
        if not is_admin(interaction.user): return
        try: member_id = int(self.user_id.value.strip())
        ...
        await interaction.response.send_message(f"✅ تم تسجيل الإنذار بنجاح.", ephemeral=True)
...
class WarningListModal(discord.ui.Modal, title="عرض إنذارات عضو"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title="الإنذارات النشطة", color=discord.Color.orange())
        embed.description = "\n".join(f"`{w['id']}` - {w['reason']}" for w in active_warnings) if active_warnings else "لا توجد إنذارات."
        await interaction.response.send_message(embed=embed, ephemeral=True)
...
class WarningRemoveModal(discord.ui.Modal, title="حذف إنذار"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        for uid, items in warnings.items():
            ol = len(items)
            warnings[uid] = [i for i in items if i["id"] != warning_id]
            if len(warnings[uid]) != ol: removed = True
        save_json(WARNING_FILE, warnings)
        await interaction.response.send_message("✅ تم الحذف بنجاح." if removed else "❌ غير موجود.", ephemeral=True)
...
async def warning_panel_cmd(ctx: commands.Context):
    if ctx.channel.id not in {WARNING_INPUT_CHANNEL, WARNING_LOG_CHANNEL} or not is_admin(ctx.author): return
    embed = discord.Embed(title="لوحة إدارة الإنذارات", description="التحكم الكامل بالإنذارات.", color=discord.Color.orange())
    await ctx.send(embed=embed, view=WarningPanel())


async def handle_warning_input(message: discord.Message):
    if message.channel.id != WARNING_INPUT_CHANNEL or message.content.startswith(tuple(bot.command_prefix)) or not is_admin(message.author): return
    if not message.mentions: return

    target = message.mentions[0]
    raw = message.content
    for m in message.mentions:
        raw = raw.replace(m.mention, "").replace(f"<@!{m.id}>", "")

    duration, reason = parse_duration_and_reason(raw)
    if duration is None: return
    if not reason: reason = "لم يتم تحديد سبب"

    warning_id, expires_at = await create_warning(message.guild, target, message.author, reason, duration, source_message_id=message.id)
    await message.reply(f"✅ تم تسجيل الإنذار بنجاح. المعرف: `{warning_id}`", mention_author=False)
...
async def handle_hacked_protection(message: discord.Message):
    if message.author.bot or (isinstance(message.author, discord.Member) and has_any_role(message.author, EXCEPTED_PROTECTION_ROLES)): return
    has_image = any(attachment.content_type and attachment.content_type.startswith("image/") for attachment in message.attachments)
    content_type_str = "🖼️ قام بإرسال صورة / مرفق" if has_image else f"📝 المحتوى:\n```{message.content}```"
    ...
    if log_channel:
        embed = discord.Embed(title="🛡️ نظام حماية السيرفر | حظر تلقائي للروم المحمي", color=discord.Color.from_rgb(200, 0, 0), timestamp=now_utc())
        embed.add_field(name="👤 العضو", value=message.author.mention, inline=True)
        embed.add_field(name="⚙️ الإجراء", value="`BAN` حظر مؤقت لحماية السيرفر", inline=False)
        embed.add_field(name="🔍 المادة المرسلة", value=content_type_str, inline=False)
        await log_channel.send(embed=embed)

    try: await message.author.send("حسابك مهكر الباند يوم")
    except discord.HTTPException: pass

    try:
        await message.guild.ban(message.author, reason="حساب مهكر", delete_message_days=1)
        tempbans = load_json(TEMPBANS_FILE)
        tempbans[str(message.author.id)] = {"guild_id": message.guild.id, "unban_at": (now_utc() + datetime.timedelta(days=1)).timestamp()}
    ...
    for uid, data in list(tempbans.items()):
        if current_timestamp >= data["unban_at"]:
            ...
            if guild:
                try:
                    user = await bot.fetch_user(int(uid))
                    await guild.unban(user, reason="انتهاء الـ 24 ساعة")
                    to_remove.append(uid)
                except discord.NotFound: to_remove.append(uid)
                except discord.HTTPException: pass
    if to_remove:
        for uid in to_remove: tempbans.pop(uid, None)
        save_json(TEMPBANS_FILE, tempbans)
...
async def on_ready():
    ...
    if not auto_reset_leaves.is_running(): auto_reset_leaves.start()
    if not award_voice_points.is_running(): award_voice_points.start()
