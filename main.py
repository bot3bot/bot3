...
class InteractionPanel(discord.ui.View):
    ...
    @discord.ui.button(label="نقاطي", style=discord.ButtonStyle.primary, custom_id="points:mine")
    ...
    @discord.ui.button(label="التوب", style=discord.ButtonStyle.success, custom_id="points:top")
    ...
    @discord.ui.button(label="إضافة نقاط", style=discord.ButtonStyle.secondary, custom_id="points:add")
    ...
    @discord.ui.button(label="تعديل ترقية", style=discord.ButtonStyle.secondary, custom_id="points:setreq")
    ...
    @discord.ui.button(label="تشغيل/إيقاف الدبل", style=discord.ButtonStyle.danger, custom_id="points:double")
...
async def handle_message_points(message: discord.Message):
    ...
    if is_points_member(message.author) and message.channel.id not in BLOCKED_CHANNELS:
        amount = 5 if load_json(DOUBLE_FILE, {"active": False}).get("active") else 2
        add_points_to_user(message.author.id, amount)
    ...
    if keyword_amount:
        ...
        await message.reply(
            f"✅ تم احتساب نقاط {keyword_name} لك: +{keyword_amount} نقطة. مجموعك الآن {total} نقطة.",
...
async def on_voice_state_update(member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
    ...
    if before.channel is not None and after.channel is None and uid in voice_times:
        ...
        if count > 0:
            amount = 30 if load_json(DOUBLE_FILE, {"active": False}).get("active") else 15
...
async def handle_warning_input(message: discord.Message):
    ...
    user_warnings = warnings.setdefault(str(target.id), [])
    ...
    log_channel = bot.get_channel(WARNING_LOG_CHANNEL)
    embed = discord.Embed(
    ...
    embed.add_field(name="المسؤول", value=message.author.mention, inline=True)
    embed.add_field(name="المدة", value=f"<t:{int(expires_at.timestamp())}:R>", inline=True)
